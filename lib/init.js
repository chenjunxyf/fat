...
const currentTmp = path.join(cwd, 'fattmp'); // 脚手架模板存放目录
const tmpDirectory = path.join(currentTmp, String((+new Date))); // clone目录
    ...
    init: function(type) {
        ...
        ps.on('exit', function(err) {
            ...
            co(function*() {
                ...
                yield remove(path.join(tplDir, '.git'));
                yield copy(tplDir, cwd);
